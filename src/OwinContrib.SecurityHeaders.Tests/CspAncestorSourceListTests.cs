...
namespace SecurityHeadersMiddleware.Tests {
    public class CspAncestorSourceListTests {
        ...
        [Fact]
        public void When_addin_one_host_multiple_times_it_should_only_be_once_in_the_header_value() {
            var sut = CreateSUT();
            sut.AddHost("http://www.example.org");
            sut.AddHost("http://www.example.org/"); //TODO See https://tools.ietf.org/html/rfc3986#section-6.2.3 to fix
            sut.ToDirectiveValue().Trim().ShouldEqual("http://www.exmple.org");
        }
